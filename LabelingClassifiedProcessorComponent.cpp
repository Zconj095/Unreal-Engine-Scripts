#include "LabelingClassifiedProcessorComponent.h"

#include "Math/UnrealMathUtility.h"

ULabelingClassifiedProcessorComponent::ULabelingClassifiedProcessorComponent()
{
    PrimaryComponentTick.bCanEverTick = false;
}

void ULabelingClassifiedProcessorComponent::BeginPlay()
{
    Super::BeginPlay();
    ...
    // Demo: argmax classification on flat probabilities
    const TArray<int32> Labels = AssignClassifiedLabelsFlat(DemoProbabilitiesFlat, DemoRows, DemoCols);
    FString LabelsStr;
    ...
    UE_LOG(LogTemp, Log, TEXT("Classified Labels: %s"), *LabelsStr);

    // Demo: tiny neural classifier
    TrainClassifiedLabelingNeuralNetwork();
}

TArray<int32> ULabelingClassifiedProcessorComponent::AssignClassifiedLabels(const TArray<TArray<float>>& Probabilities)
{
    TArray<int32> Labels;
...
TArray<int32> ULabelingClassifiedProcessorComponent::AssignClassifiedLabelsFlat(const TArray<float>& FlatRowMajor, int32 Rows, int32 Cols)
{
    TArray<int32> Labels;
    if (Rows <= 0 || Cols <= 0)
    {
        UE_LOG(LogTemp, Error, TEXT("AssignClassifiedLabelsFlat: Rows and Cols must be > 0."));
        return Labels;
    }
    if (FlatRowMajor.Num() != Rows * Cols)
    {
        UE_LOG(LogTemp, Error, TEXT("AssignClassifiedLabelsFlat: Flat array size %d != Rows*Cols %d."), FlatRowMajor.Num(), Rows * Cols);
        return Labels;
    }
...
int32 ULabelingClassifiedProcessorComponent::ArgMax(const float* Data, int32 Count)
{
    if (Count <= 0)
    {
        return 0;
...
ULabelingClassifiedProcessorComponent::FMLP3C::FMLP3C()
{
...
float ULabelingClassifiedProcessorComponent::FMLP3C::Sigmoid(float X)
{
    return 1.0f / (1.0f + FMath::Exp(-X));
}

float ULabelingClassifiedProcessorComponent::FMLP3C::SigmoidDerivFromY(float Y)
{
    return Y * (1.0f - Y);
}

void ULabelingClassifiedProcessorComponent::FMLP3C::Forward(const float* Input, float* OutHidden, float* OutOutput) const
{
    for (int32 h = 0; h < HiddenSize; ++h)
    {
        float Z = B1[h];
...
void ULabelingClassifiedProcessorComponent::FMLP3C::TrainSGD(const float* Input, const float* Target, float LearningRate)
{
...
void ULabelingClassifiedProcessorComponent::TrainClassifiedLabelingNeuralNetwork(int32 MaxEpochs, float LearningRate, float ErrorThreshold)
{
